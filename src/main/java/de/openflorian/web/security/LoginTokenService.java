...
public class LoginTokenService {
	...
	public String createFrontendLoginToken(User u) {
		String plainToken = CryptCipherService.service()
				.encrypt(
						PRINCIPAL_ID_PREFIX + "="
				...
				+ CryptCipherService.service().encrypt(
						TTL_PREFIX + "=" + String.valueOf((System.currentTimeMillis() / 1000) + TOKEN_TTL),
						CipherTarget.Blowfish);
		return CryptCipherService.service().encrypt(plainToken, CipherTarget.Xor);
	}
	...
	public String createActorToken(User u) {
		String plainToken = CryptCipherService.service().encrypt(PRINCIPAL_ID_PREFIX + "=" + String.valueOf(u.getId()),
				CipherTarget.Blowfish);
		return CryptCipherService.service().encrypt(plainToken, CipherTarget.Xor);
	}
	...
	public long extractActorIdFromToken(String actorToken) throws IllegalArgumentException {
		try {
			String plainToken = CryptCipherService.service().decrypt(actorToken, CipherTarget.Xor);
			return Long.parseLong(CryptCipherService.service().decrypt(plainToken, CipherTarget.Blowfish)
					.substring(PRINCIPAL_ID_PREFIX.length() + 1));
		} catch (Exception e) {
	...
	public boolean validateFrontendLoginToken(User u, String token) {
		try {
			String plainToken = CryptCipherService.service().decrypt(token, CipherTarget.Xor);
			String[] stoken = plainToken.split(";");

			if (stoken.length != 2)
				return false;

			String principalstr = CryptCipherService.service().decrypt(stoken[0], CipherTarget.Blowfish);
			String ttlstr = CryptCipherService.service().decrypt(stoken[1], CipherTarget.Blowfish);
