...
public abstract class GenericDao<T, ID extends Serializable> {
	...
	public GenericDao() {
		...
		this.primaryKeyType = (Class<ID>) ((ParameterizedType) getClass().getGenericSuperclass())
	...
	public static EntityManager getLocalEntityManager() {
		if (emf == null)
			emf = Persistence.createEntityManagerFactory("openflorian");

		if (localEm == null)
			localEm = new ThreadLocal<EntityManager>();

		// Recyle entity manager in same thread, create new if em was closed by
		// JpaFilter
	...
	/**
	 * Trys to load a local entity manager set by dependency injection. If local
	 * entity manager is not found, we try to get the XML defined entity
	 * manager.
	 * 
	...
	public EntityManager getEntityManager() {
		if (this.em == null)
			return GenericDao.getLocalEntityManager();
		else
			return this.em;
	}

	/**
	 * Find all in specific order. Please don't implement findAll() without
	 * order parameter. If you really need to please ask the original author.
	 * 
	 * @param order
	...
	public List<T> findAll(String order) {
		return (List<T>) getEntityManager().createQuery("from " + entityType.getCanonicalName() + " order by " + order)
				.getResultList();
	}
	...
	public List<T> findAllByPage(String order, int pagesize, int offset) {
		return (List<T>) getEntityManager().createQuery("from " + entityType.getCanonicalName() + " order by " + order)
				.setFirstResult(pagesize * offset).setMaxResults(pagesize).getResultList();
	}
	...
	public List<T> findAllByAttribute(String attribute, Object value) {
		Query query = getEntityManager()
				.createQuery("from " + entityType.getCanonicalName() + " o WHERE o." + attribute + " LIKE :value");
	...
	public T findByAttribute(String attribute, Object filter) {
		Query query = getEntityManager()
				.createQuery("from " + entityType.getCanonicalName() + " o WHERE o." + attribute + " = :value");
	...
	/**
	 * Persist object. Persist means: The object is updated if it was loaded
	 * from the db, otherwise inserted. Should be _the_ method for saving
	 * objects.
	 * 
	 * @param object
	 *            Bean
	...
	/**
	 * Merge object. Merge means: The object is merged with the one from db.
	 * Should be used only in rare cases where objects are evicted before.
	 * 
